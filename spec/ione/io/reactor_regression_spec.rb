...
module Ione
  module Io
    describe(IoReactor) do
      ...
      before { @existing_threads = ::Thread.list }
      ...
      def reactor_threads
        ::Thread.list.select {|t| t.name == IoReactor::THREAD_NAME && !@existing_threads.include?(t)}
      end
    ...
    describe(IoLoopBody) do
      ...
      let(:scheduler) { Scheduler.new(clock: clock) }
      let(:io_loop) do
        IoLoopBody.new(unblocker, scheduler: scheduler,
